...
object Rng {
  ...
  case class Deterministic(first: Int, rest: Int*) extends Rng {
    override def nextInt: (Int, Rng) = {
      val (newFirst, newRest) = rest match {
        case Seq() => (first, Seq())
        case Seq(head, tail@_*) => (head, tail.appended(first))
      }
      (first, Deterministic(newFirst, newRest: _*))
    }
  }
